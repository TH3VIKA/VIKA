...
reload_plugins() 
return " 🚸 ~>  تہ‌‏م تحديث مـلفآت البـوت بـنجاح\n✓"
end 
if (matches[1] == "sp" or matches[1] == "جلب ملف") and is_sudo(msg) then 
if not we_sudo(msg) then return "‼️ ~> هذا الاوامر للمطور الاساسي فقط 🛠" end
if (matches[2]=="الكل" or matches[2]=="all") then
sendMsg(msg.to.id,msg.id_,'🗂*🚸 ~>* انتضر قليلا سوف يتم ارسالك كل الملفات','md')
for k, v in pairs(plugins_names()) do  
sendDocument(msg.to.id, msg.id_,0, 1, nil, "./plugins/"..string.match (v, "(.*)%.lua") ..".lua", '🗂 ~> آلمـلف مـقدم مـن قنآ‏‏هہ فيـكأا ¦ֆ \n📢 ~> تابع قناة السورس @VikaiQ\n👨🏽‍🔧', dl_cb, nil) end  else
if not plugin_exists(matches[2]) then 
return '🗂*~>* لا يوجد ملف بهذا الاسم .\n\n'
else sendDocument(msg.to.id, msg.id_,0,1,nil,"./plugins/"..matches[2]..".lua", '🗂 ~> آلمـلف مـقدم مـن قنآ‏‏هہ فيـكأا ¦ֆ \n📢 ~> تابع قناة السورس @VikaiQ\n👨🏽‍🔧', dl_cb, nil)
end end end
...
return '👨🏾‍🔧¦ اصدار سـورس فيـكاا : `v'..update..'` \n📡'
end
if (matches[1] == 'تحديث السورس' or matches[1] == 'تحديث السورس ™') and is_sudo(msg) then
if not we_sudo(msg)  then return "*~> * هذا الاوامر للمطور الاساسي فقط ‼️" end
return"*• * انتهى دعم هذا السورس بالتحديثات ترقبو السورس الجديد وبالتيجي الجديد في قناة فيـكاا @VikaiQ . \n📦"
end
...
echo '☑️» Seystem : `'"$linux_version"'`' 
echo '☑️» Memory Total : `'"$memTotal_m MB"'`'
echo '☑️» Memory Used : `'"$memUsed_m MB ~ $memUsedPrc% used!"'`'
echo '☑️» Disk Total : `'"$disk_total"' For disk.`'
echo '☑️» Disk Used : `'"$disk_used"' used!`'
echo '☑️» Disk Free : `'"$disk_free"' is free.`'
echo '☑️» CPU Usage : `'"$CPUPer"'%`'
echo '☑️» Processes : `Core'"$ProcessCnt"'`'
echo '☑️» Login : `'"`whoami`"'`'
