...
return "👋🏻*~>>*   اهــلا بـك عزيزيي \n📌*~>>*  تـم تثبيت الرسالـه بنجـاح  \n★"
end
...
return "👋🏻*~>>*   اهــلا بـك عزيزيي \n📌*~>>*  تـم الغـاء تثبيت الرسالـه بنجـاح  \n★"
end
...
if not redis:get(boss..'linkgp'..msg.to.id) then return "🔗*~>>*  لا يوجد هنا رابط\n🔖¦ *رجائا اكتب [ضع رابط]*🔃" end
...
return '🔖*~>>* عدد رسائلك عزيزيي هي  (*'..tonumber(redis:get(boss..'msgs:'..msg.from.id..':'..msg.to.id) or 0)..'*) رساله \n' end
 if matches[1]== 'جهاتي' then
 return '📎*~>>*  عدد جهاتك التي قمت باضافتها   » 【'..(tonumber(redis:get(boss..':addusers_group:'..msg.to.id..':'..msg.from.id)) or 0)..'】 . \n——'
end
...
sendMsg(msg.to.id,0,'🗣*¦*  تم اذاعه الكليشه الى *'..#groups..'* مــجمـوعهہ‏‏\n✓')			
end end end
